	...
	public void Enter(S_Entity enterer, bool instant) {
		if (enterer is S_Player && content != null) {
			if (instant) {
				...
				slider.SlideTo((int)enterer.transform.position.x - S_Tile.width / 2, (int)enterer.transform.position.y - S_Tile.height / 2, dist / 70000f,  () => { Game.Unlock(); Enter(enterer, true); });
			}
		}
